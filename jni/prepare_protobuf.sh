...
# Function to copy protobuf from subprojects (using correct path)
copy_protobuf_from_subprojects() {
  local subprojects_dir="../subprojects/protobuf-${VERSION}"
...
# Function to copy abseil-cpp from subprojects (using correct path)
copy_abseil_from_subprojects() {
  local absl_dir="../subprojects/abseil-cpp-20250814.1"
  echo "[PROTOBUF] Checking abseil-cpp directory: ${absl_dir}"
  if [ -d "${absl_dir}" ]; then
    echo "[PROTOBUF] Found abseil-cpp at ${absl_dir}"
    mkdir -p "protobuf-${VERSION}/third_party/abseil-cpp"
    # Copy the contents of abseil-cpp directory
    cp -r "${absl_dir}/"* "protobuf-${VERSION}/third_party/abseil-cpp/"
    return 0
  else
    echo "[PROTOBUF] abseil-cpp not found at expected location: ${absl_dir}"
    return 1
  fi
}
...
if [ ! -d "protobuf-${VERSION}" ]; then
    if [ ! -f "protobuf-${VERSION}.tar.xz" ]; then
      ...
      if ! wget -q ${URL} ; then
        ...
        if ! copy_protobuf_from_subprojects; then
...
if [ ! -z "$NDK_PATH" ] && [ -d "$NDK_PATH" ]; then
  ...
  if [ ! -f "${TARGET}/protobuf-${VERSION}/lib/arm64-v8a/libprotobuf.a" ]; then
    ...
    # Copy utf8_range library
    echo "[PROTOBUF] Copying utf8_range library"
    if [ -f "protobuf-${VERSION}/build/arm64-v8a/third_party/utf8_range/libutf8_validity.a" ]; then
      echo "[PROTOBUF] Found utf8_range library"
      mkdir -p protobuf-${VERSION}/lib/arm64-v8a
      cp protobuf-${VERSION}/build/arm64-v8a/third_party/utf8_range/libutf8_validity.a protobuf-${VERSION}/lib/arm64-v8a/
    else
      echo "[PROTOBUF] utf8_range library not found"
    fi
